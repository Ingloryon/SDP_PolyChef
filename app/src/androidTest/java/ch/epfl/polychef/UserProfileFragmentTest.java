...
public class UserProfileFragmentTest {
    ...
    @Test
    public synchronized void userRecipesAreDisplayedWhenThereAreFewOfThem() throws InterruptedException {
        testUserRecipes(0, UserProfileFragment.nbOfRecipesLoadedAtATime + 1);
    }

    @Test
    public synchronized void userRecipesAreDisplayedWhenThereAreManyOfThem() throws InterruptedException {
    ...
    public synchronized void testUserRecipes(int min, int max) throws InterruptedException {
        ...
        assertEquals(Math.min(recipeLoaded, n), getTestedFragment().getRecyclerView().getAdapter().getItemCount());

        for(int i = 0; i < n/recipeLoaded; ++i){
            wait(1000);

            int scrollTo = getTestedFragment().getRecyclerView().getAdapter().getItemCount() - 1;
        ...
        assertEquals(n, getTestedFragment().getRecyclerView().getAdapter().getItemCount());
    }

    @Test
    public void noRecipesAreDisplayedWhenUserHasNoRecipes() {
        startTest();
        assertEquals(0, getTestedFragment().getRecyclerView().getAdapter().getItemCount());
    }
